...
def generate_pixels(img, SLIC_height, SLIC_width, SLIC_ITERATIONS, SLIC_centers, SLIC_labimg, SLIC_m, SLIC_clusters, step):
...
def calculate_centers(step, SLIC_width, SLIC_height, SLIC_labimg):
    ...
    for i in range(step, SLIC_width - int(step/2), step):
        for j in range(step, SLIC_height - int(step/2), step):
            nc = find_local_minimum(center=(i, j), SLIC_labimg=SLIC_labimg)
...
def process(img_path):
    ...
    step = int((img.shape[0]*img.shape[1]/int(num_pixels))**0.5)
    SLIC_m = compactness
    SLIC_ITERATIONS = 4
    ...
    # main
    generate_pixels(img, SLIC_height, SLIC_width, SLIC_ITERATIONS, SLIC_centers, SLIC_labimg, SLIC_m, SLIC_clusters, step)
    create_connectivity(img, SLIC_width, SLIC_height, SLIC_centers, SLIC_clusters)
    calculate_centers(step, SLIC_width, SLIC_height, SLIC_labimg)
    display_contours(img, SLIC_width, SLIC_height, SLIC_clusters, color)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('data/U2OS_train_small.csv')
    ...
    img_path = os.path.join(img_dir, mode, experiment, f'Plate{plate}', f'{well}_s{site}_w{channel}.png')
    process('Lenna.png')